module Page.Radio exposing (page)
...
description : String
description =
    "Radioes toggle the state of a single item on or off."
...
viewFunctions =
    let
        viewRadio style desc selected onPress { palette } () =
            Widget.radio (style palette)
                { description = desc
                , selected = selected
                , onPress = onPress
                }
                --Don't forget to change the title
                |> Page.viewTile "Widget.radio"
    in
    [ viewRadio ]
...
view { palette } (IsButtonEnabled isButtonEnabled) =
    Widget.radio (Material.radio palette)
